...
(function($){
    ...
    $.fn.wBox = function(options){
		...
		var wBox_btnArea = "";
		// alert("xx x  "+options.isDisBtn);
		if(options.isDisBtn==1){
			wBox_btnArea ='<div id="wBox_btnArea" class="wBox_btnArea"><input type="button" class ="wboxButton wBox_submit" value="确定">&nbsp;&nbsp;&nbsp;&nbsp;<input type="button"  class ="wboxButton wBox_close" value="取消"></div>';
		}
        ...
        '<div class="wBox_content" id="wBoxContent"></div>'+
        wBox_btnArea
        +'</div></td><td class="wBox_b"><div style="width:10px;">&nbsp;</div></td></tr><tr><td class="wBox_bl"/><td class="wBox_b"/><td class="wBox_br"/></tr></tbody></table></div></div>', B = null, C = null, $win = $(window),$t=$(this);//B背景，C内容jquery div   
        this.showBox=function (target){        	
        	if(target!= undefined && target!=""){ 
        		_this.YQ.target = target
        	}
            $("#wBox_overlay").remove();
			$("#wBox").remove();
        ...
        function handleClick(){
			...
			if (_this.YQ.requestType && $.inArray(_this.YQ.requestType, ['iframe', 'ajax','img',"div"])!=-1) {
				con.html("<div class='wBox_load'><div id='wBox_loading'><img src='"+_this.YQ.wBoxURL+"loading.gif' /></div></div>");
				
				// alert("_this.YQ.requestType:" + _this.YQ.requestType);
				if (_this.YQ.requestType === "img") {
					...
					if (_this.YQ.requestType === "div") {
							var htm= '<div id="wBoxContentInFrame" style="margin-top:50px;font-size:20px;text-align:center;vertical-align:middle;width:400px;height:50px"><span id="wBoxContentInFrame-content">'+_this.YQ.msg+'</span></div>'		
							con.html(htm);
							//alert(con.html());
							C.find('.wBox_close').click(_this.close);
							C.find('.wBox_submit').click(_this.confirmSubmit);
							setPosition();
					}
				else 
					if (_this.YQ.requestType === "ajax") {
						...
						ifr.load(function(){
							try {
								...
								$it.find('.wBox_submit').click(_this.submit);
								// alert($it.find('#company').val())
        ...
        this.confirmSubmit=function (){
        	_this.YQ.successFunc();
            // alert("ty");
            //_this.YQ.confirmRet = true;            
        }
        ...
        this.submit=function (){
            // alert("submit23..." + _this.YQ.successFunc)
            _this.YQ.successFunc();
        }
        /*
         * 触发click事件
