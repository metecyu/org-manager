...
//正数验证       可空
jQuery.validator.addMethod("positiveNull", function(value, element) {
	if (value > 0 ||value=="") {
		return true;
	} else {
		return false;
...
//确认与指定输入框内容相同
jQuery.validator.addMethod("affirmValue", function(value, element, param) {
	var newPwd = $(param).val();
	if(newPwd != "") {
		return value == newPwd;
	}else{
		return true;
	}	
}, $.validator.format("与指定输入框内容不一致"));
