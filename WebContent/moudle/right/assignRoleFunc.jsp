...
<script> 
	...
	var setting = {
		...
		callback: {
			onClick: function (event, treeId, treeNode) {	
				...
				form.action = "${ctx}/right/beginAssignRoleFunc.do?roleid="+treeNode.id;
				form.method = "post";
				form.submit();
			}
		}
	...
	var zNodesFunc =[
           <c:forEach items="${funcList}" var="obj" varStatus="status">
           		{ id:"${obj.id}", pId:"${obj.pid}", name:"${obj.funcname}", checked:"${obj.checked}",open:true, nocheck:false},
           </c:forEach>
	...
	$().ready(function() {
		...
		// ==========functree==========
		settingFunc.check.chkboxType = { "Y" : "", "N" : ""}; //设置选择子功能选择项时，不选择父功能项（ztree官网有介绍） //"Y" : "s", "N" : "s"
	...
	function assignUserRole(){	
		...
		$.ajax({    
            url : '${ctx}/right/updateRoleFunc.do',    
            cache : false,    
            type : 'post',    
            dataType : 'html',    
            async : false,    
